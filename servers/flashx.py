...
def get_video_url(page_url, premium=False, user="", password="", video_password=""):
    ...
    msg_convert = scrapertools.find_single_match(data,'<div id="over_player_msg">([^"]+)<br>')
    if msg_convert != "":
        error = xbmcgui.Dialog()
        error.ok("Vídeo no disponible", "El vídeo se está convirtiendo")
        return None
...
def find_videos(data):
    ...
    patronvideos = '//(?:www.|play.)?flashx.tv/(?:embed-|dl\?)?([0-9a-zA-Z/-]{12})'
